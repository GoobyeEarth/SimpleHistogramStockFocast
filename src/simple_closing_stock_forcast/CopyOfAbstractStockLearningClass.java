...
public class CopyOfAbstractStockLearningClass {
...
//
		...
		for(; index < data.size() && data.get(index).pastX <= influenceDistEnd; index++){
			int mdp;
			int mdFromStart = (int) (data.get(index).pastX - influenceDistStart + 1);
...
//		if(index < data.size() ) System.out.println("last pastX:" + data.get(index).pastX);
	...
	private int getExpectedDataPlace(float pastX, float first, float last){
		...
		int ep;
		if (4 < data.size()) {
			if(pastX < mid) ep = (int) (data.size() * (pastX - first) / d);
			else ep = (int) (data.size() - (data.size() * (pastX - last) / d));

			if(ep < 1) ep = 1;
			if( data.size() <= ep) ep  = data.size()-2;
		}
		else{
			ep = 1;
		}
